...
selectOption = list(pyautogui.locateAllOnScreen('selectOption.png', confidence = 0.90))
...
chooseOption = list(pyautogui.locateAllOnScreen('chooseOption.png', confidence=0.85))
...
time.sleep(0.2)
pyautogui.click(backMonthSelection[0] - 50, backMonthSelection[1], duration = 0.10, clicks=3) 
...
pyautogui.click(ask[0] + 30, ask[1] + 50, duration=0.15) 


# Copy and extract the option details for the Front Month
time.sleep(0.2)
pyautogui.click(frontMonthSelection[0] - 50, frontMonthSelection[1], duration = 0.10, clicks=3) 
pyautogui.hotkey('ctrl', 'c')
time.sleep(.05) 
...
pyautogui.click(centerText[0], centerText[1] - 20, duration=0.10) 
...
for i, line in enumerate(f):
    ...
    if i == 1:
        ...
        entryCost = float(line[13 : line.find('(')]) 
        
        if 'net debit' in line: entryCost = entryCost * -1
        
        
    if i == 3:
        maxRisk = float(line[15 : line.find('at a') - 1])
    
    
    if i == 5:
        maxReturn.append(float(line[17 : line.find('at a') - 1]))
        maxReturn.append(float(line[line.find('price of') + 10 : line.find('at exp') - 1]))
...
negScore = round(negReturns / negSums, 2)
...
overallScore = ((adjAvg * 2) + ((posScore + negScore) * 2) + ((posSums / overallNums) * 2) + (maxReturnScore * 2) - (breakevenScore * 2)) / 3
...
print('\nScore of Negative Returns:', negScore, round(negSums / overallNums, 2))
print('\nMax Return & Score:', maxReturn[0], maxReturnScore)
print('\nBreakevens & Score:', breakevens, round(breakevenScore, 2))
